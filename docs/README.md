...
- Output errors and lint failures in a common `file:line:column:message` format, making integration
  ...
  > 以通用`file:line:column:message`格式输出错误和lint失败，为了使与编辑器的集成成为可能，Vim integration是开箱即用的
...
> Prototool通过为您实时下载并调用`protoc`来实现这一点，处理来自`protoc`和您的插件的错误消息，并使用生成的`FileDescriptorSet`实现内部功能，以及包装Protobuf生态系统中已有的一些优秀的外部库。编译、检测和格式化命令处理一个protobuf文件在0.03秒，或者处理一个大数量的500个以上protobuf文件在1秒以内
...
[mit]: https://github.com/gongxulei/prototool/blob/master/LICENSE

[release-img]: https://img.shields.io/github/release/uber/prototool/all.svg
[release]: https://github.com/gongxulei/prototool/releases
